...
% COLOR_GRADIENT computes the vector gradient of a color image.
%  [VG, A, PPG] = COLOR_GRADIENT(f, T) computes the vector gradient, VG,
%  and corresponding angle array, A, (in radians) of RGB image f. It also
...
%  the range [0, 1]. If it is included in the argument list, the values of
%  VG and PPF are thresholded by letting VG(x,y) = 0 for values <= T and
%  VG(x,y) = VG(x,y) otherwise. Similar comments apply to PPG. If T is not
%  included in the argument list then T is set to 0. Both output gradients
...
GG = sqrt(Gx.^2 + Gy.^2);
BG = sqrt(Bx.^2 + By.^2);
% Form the composite by adding the individual results and sacle to [0, 1].
